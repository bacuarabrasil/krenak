...
def test_user_profile_api_get_success(user_account, api_client):
    ...
    user = user_account(email=email, first_name=first_name, last_name=last_name, birthdate=birthdate)
    client = api_client(auth_user=user)

    response = client.get(reverse("api-v1-accounts:user-profile"))

    assert response.status_code == status.HTTP_200_OK
    assert response.data == {"email": email, "first_name": first_name, "last_name": last_name, "birthdate": birthdate}
...
def test_user_profile_api_update_success(user_account, api_client):
    ...
    user = user_account(email=email, first_name=old_first_name, last_name=old_last_name, birthdate=birthdate)
    client = api_client(auth_user=user)
